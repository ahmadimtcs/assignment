...
public class WeatherServiceImpl implements WeatherService {
	...
	public Mono<WeatherDto> createWeatherDetails(Mono<WeatherDto> weatherDto) {
		...
		generateWeatherDataService.refreshAfterTime();
		return weatherDto.map(m -> modelMapper.map(m, WeatherDetails.class)).flatMap(weatherRepository::save)
	...
	public Mono<WeatherDto> updateWeatherDetails(Long id, Mono<WeatherDto> weatherDto) {
		...
		return weatherRepository.findById(id)
				.flatMap(result -> weatherDto.map(m -> modelMapper.map(m, WeatherDetails.class))
						.flatMap(weatherRepository::save).map(m -> modelMapper.map(m, WeatherDto.class)));
	}
