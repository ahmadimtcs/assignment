...
class WeatherControllerTest {
	...
	void getAllWeatherDetails() {
		...
		webTestClient.get().uri(WEATHER_END_POINT).exchange().expectStatus().isOk().expectBody().jsonPath("[0].name")
				.isEqualTo("Mumbai");
	}
	...
	void createWeatherDetails() {
		...
		webTestClient.post().uri(WEATHER_END_POINT).bodyValue(weatherDetails).exchange().expectStatus().isOk()
	...
	void updateWeather() {
		...
		Mockito.when(weatherRepository.save(weatherDetails)).thenReturn(Mono.just(weatherDetails));
		webTestClient.put().uri(WEATHER_END_POINT.concat("/{id}"), 8131499L).bodyValue(weatherDetails).exchange()
				.expectStatus().isOk().expectBody(WeatherDetails.class);
	...
	void getWeatherDetailsByCity() {
		...
		webTestClient.get().uri(WEATHER_END_POINT.concat("/city/{city}"), "Mumbai").exchange().expectStatus().isOk()
				.expectBody().jsonPath("$.name").isEqualTo("Mumbai");
	...
	void getCityNotFoundException() {
		...
		webTestClient.get().uri(WEATHER_END_POINT.concat("/city/{city}"), "Mumbai").exchange().expectStatus()
				.isNotFound();
	}
	...
	void deleteWeatherDetails() {
		...
		webTestClient.delete().uri(WEATHER_END_POINT.concat("/{id}"), 8131499L).accept(MediaType.APPLICATION_JSON)
